...
API cho nền tảng quản lý học tập - hai vai trò trong mỗi CSGD là Giáo viên
(toàn quyền quản lý CSGD: lớp học, thành viên, học liệu chung, thông báo -
cộng với giảng dạy) và Học sinh, cùng Quản trị hệ thống ở tầng nền tảng
(duyệt CSGD đăng ký mới). Xem tài liệu thiết kế đầy đủ tại `lms-design.html`
ở thư mục gốc repo.
...
1. `POST /auth/register-institution` - giáo viên tự đăng ký CSGD (tạo user + institution + membership `teacher` trong 1 transaction), trạng thái `pending`. Người đăng ký trở thành giáo viên có toàn quyền quản lý CSGD này.
2. `PATCH /institutions/:id/approve` - system_admin duyệt → CSGD chuyển `active`.
3. `POST /institutions/:institutionId/members` - giáo viên thêm giáo viên khác/học sinh.
